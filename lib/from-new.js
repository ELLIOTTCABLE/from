...
;(function(){ if(typeof(from)!=="undefined"&&from){module.exports=from;return from}; return (function(){
  ...
  , $$ = function($){ var $ = $.slice(0)
      if (!$[-1].paths) {
        return $[-1].resolver.call(it
        , $[-2].paths.lastThat(function(path){ return path.resolvers.indexOf($[-1]) !== -1 }).path
        , query, function(error, paths){ $[-1].error = error; $[-1].paths =
          paths.map(function(path){ return{ path:path
                                    , resolvers:$[-1].resolver.after.map(function(resolver){ return{
                                                  resolver:resolver, paths:undefined, error:undefined } })
      ...
      if (!unfinishedPaths.empty) return $$(($.push(unfinishedPaths.first.resolvers
                                           .firstThat(function(resolver){ return !resolver.paths })), $))
      
      if (!untestedPaths.empty)
        return probe(untestedPaths.first.path, function(e, extant){ if (!e) return _(u, extant, $)
