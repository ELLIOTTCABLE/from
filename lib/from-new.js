...
;(function(){ if(typeof(from)!=="undefined"&&from){module.exports=from;return from}; return (function(){
  ...
  , $$ = function($){
      console.error(".. recursing! (stack: "+$.length+", stack.last: "+$[-1].resolver.tag+")")
      ...
      if (!$[-1].paths) {
        ...
        return $[-1].resolver.call(it, path.path, query, function(error, paths){ var
          after = $[-1].resolver.after.map(function(resolver){
            return{ resolver:resolver, paths:undefined, error:undefined } })
          $[-1].error = error
          $[-1].paths = paths.map(function(path){ return{ path:path, resolvers:after, error:undefined } })
          return $$($)
