...
;(function(){ if(typeof(from)!=="undefined"&&from){module.exports=from;return from}; return (function(){
  ...
  mutate = function(path, mutator, _) { var that = this, iterate
    ;(iterate = function(stack){ var sink, tip = stack[stack.length - 1], up = stack[stack.length - 2]
      tip.mutator.call(that.from, up? up.paths[0]:path, path, (climb = function(e, paths){
        if (e && e.critical !== false) return _(e)
        probe((tip.paths = paths)[0], function(e, extant){ // if (e) return _(e)
          ...
          ;(sink = function(){
            ...
            if (!up) return _(new(Error)("Path could not be resolved"))
  ...
  , dir = path.slice(0, (i = path.indexOf(query))), rest = path.slice(i)
    fs.stat(dir, function(e, stat){ if (e) return _((e.critical = false, e))
      if (stat.isDirectory()) fs.readdir(dir, function(e, subdirs){ if (e) return _(e)
        _(u, subdirs.map(function(subdir){ return dir+"/"+subdir+"/"+rest })) })
      else return _(((e = new(Error)("Path does not refer to a directory")).critical = false, e)) }) }
  , { after:[extensionMut, initializerMut, libMut] })
  ...
  repositoryMut = new(ResolutionMutator)(function(path, query, _){
    _(u, (this.repositories || require.paths).map(function(repository){ return repository+"/"+path })) }
  , { after:[extensionMut, initializerMut, libMut, subdirMut] })
  ...
  relativeMut = new(ResolutionMutator)(function(path, query, _){ var e
    if (!this.filename)
      _(((e = new(Error)("Cannot resolve relative paths outside from’s purview")).critical = false, e))
    _(u, [path]) }, { after:[extensionMut, initializerMut, libMut] })
