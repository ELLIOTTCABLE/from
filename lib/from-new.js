...
;(function(){ if(typeof(from)!=="undefined"&&from){module.exports=from;return from}; return (function(){
  ...
  Acquisition.prototype.complete = function(){ var that = this
    args = that.queries.reduce(function(args, query){
      args.push.apply(args, query.queries.reduce(function(args, q){
        args.push.apply(query.imports? Object.keys(query.imports).map(function(key){
  ...
  from = function(args){
    ...
    , queries = Array.prototype.slice.call(args)
    
    that.queries.push({ queries:Array.prototype.slice.call(args) })
