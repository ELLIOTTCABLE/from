...
;(function(){ if(typeof(from)!=="undefined"&&from){module.exports=from;return from}; return (function(){
  ...
  Acquisition.prototype.dispatch = function(){ var that = this, iterate, jterate
    ...
    ;(iterate = function(index){ if (index >= that.groups.length) return that.complete()
      ;(jterate = function(jndex){ if (jndex >= that.groups[index].queries.length) return iterate(++index)
        execute.call(that
        , that.groups[index].queries[jndex], that.from.rootMutators, that.groups[index].exports, null
        , function(e, result){ if (e) throw e
            if (that.groups[index].imports) {
              that.from.imports[that.groups[index].queries[jndex]] = new(Object)()
              Object.keys(that.groups[index].imports).forEach(function(key){
                 that.from.imports[that.groups[index].queries[jndex]][key] =
                   result[that.groups[jndex].imports[key]] }) }
            else that.from.imports[that.groups[index].queries[jndex]] =
                   result
  ...
  Acquisition.prototype.complete = function(){ var that = this
    args = that.groups.reduce(function(args, group){
      args.push.apply(args, group.queries.reduce(function(args, q){
        args.push.apply(group.imports? Object.keys(group.imports).map(function(key){
 ...
 _export = function(){
    this.groups.last.exports = Array.prototype.slice.call(arguments)
    return this }                                                                 ;Acquisition.prototype.import =
 _import = function(){
    this.groups.last.imports = Array.prototype.slice.call(arguments)
    return this }
