...
;(function(){ if(typeof(from)!=="undefined"&&from){module.exports=from;return from}; return (function(){
  ...
  ;(function(){ var super = Function
    ;((From = function(blueprint){ var that
      (that = function(){return from.apply(this, arguments)})['__proto__'] = From.prototype
  ...
  ;(function(){ var super = Function
    ;((Acquisition = function(blueprint){ var that
      (that = function(){return acquisition.apply(this, arguments)})['__proto__'] = Acquisition.prototype
  ...
  mutate = function(path, mutator, _) { var that = this, iterate
    ;(iterate = function(stack){ var sink, tip = stack[stack.length - 1], up = stack[stack.length - 2]
      tip.mutator.call(that, up? up.paths[0]:path, path, (climb = function(e, paths){ if (e) return _(e)
          probe((tip.paths = paths)[0], function(e, extant){ // if (e) return _(e)
            console.log(new(Array)(stack.length * 2).join(" ") + tip.paths[0])
            if (extant) return _(u, extant)
            
            if (tip.mutator.after && tip.mutator.after.length > 0) {
              stack[stack.length] = { mutator:tip.mutator.after[0], paths:undefined }
              return iterate(stack) }
            
            ;(sink = function(){ if (!up) return _(new(Error)("Path could not be resolved"))
              if (tip.paths.length > 1) return climb(u, tip.paths.slice(1))
              
              if ((index = up.mutator.after.indexOf(tip.mutator)) < up.mutator.after.length - 1) {
                stack[stack.length - 1] = { mutator:up.mutator.after[index + 1], paths:undefined }
                return iterate(stack) }
              
              --stack.length, tip = stack[stack.length - 1], up = stack[stack.length - 2]
              sink() })() }) })) }) ([{ mutator:mutator, paths:undefined }]) }
  ...
  extensionMut = new(ResolutionMutator)( function(path, query, _){ var
    paths = this.extensions.map(function(extension){ return path+extension })
    _(u, paths) }, { after:[] } )                                                ;From.prototype.initializerMut =
  initializerMut = new(ResolutionMutator)( function(path, query, _){
    path = path+"/"+path.slice(path.lastIndexOf('/') + 1)
    _(u, [path]) }, { after:[extensionMut] } )                                           ;From.prototype.libMut =
  libMut = new(ResolutionMutator)( function(path, query, _){ _(u, [path]) } )         ;From.prototype.subdirMut =
  subdirMut = new(ResolutionMutator)( function(path, query, _){ _(u, [path]) } )  ;From.prototype.repositoryMut =
  repositoryMut = new(ResolutionMutator)( function(path, query, _){ _(u, [path]) } );From.prototype.relativeMut =
  relativeMut = new(ResolutionMutator)( function(path, query, _){ _(u, [path]) } )
  
  /* This mutator is a noop. It’s simply here to ensure that the first series of resolution checks is always
     applied to the raw path queried. */                                            ;From.prototype.absoluteMut =
  absoluteMut = new(ResolutionMutator)(function(path, query, _){
    _(u, [path]) }, { after:[extensionMut, initializerMut] })
