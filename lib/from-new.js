...
;(function(){ if(typeof(from)!=="undefined"&&from){module.exports=from;return from}; return (function(){
  ...
  mutate = function(path, mutator, _) { var that = this, iterate
    ;(iterate = function(stack){ var sink, tip = stack[stack.length - 1], up = stack[stack.length - 2]
      tip.mutator.call(that, up? up.path:path, path, function(e, path){ if (e) return _(e)
        probe(tip.path = path, function(e, extant){ // if (e) return _(e)
          if (extant) return _(u, extant)
          
          if (tip.mutator.after && tip.mutator.after.length > 0) {
            stack[stack.length] = { mutator:tip.mutator.after[0], path:undefined }
          ...
          ;(sink = function(){ if (!up) return _(new(Error)("Path could not be resolved"))
            if ((index = up.mutator.after.indexOf(tip.mutator)) < up.mutator.after.length - 1) {
              stack[stack.length - 1] = { mutator:up.mutator.after[index + 1], path:null }
              return iterate(stack) }
            
            --stack.length, tip = stack[stack.length - 1], up = stack[stack.length - 2]
            sink() })() }) }) }) ([{ mutator:mutator, path:undefined }]) }
  ...
  extensionMut = new(ResolutionMutator)( function(path, query, _){
    path = this.extensions.map(function(extension){ return path+extension })[0]
    _(u, path) }, { after:[] } )                                                 ;From.prototype.initializerMut =
