var fs = require('fs')
,   vm = require('vm')
, pfad = require('path'), p = pfad.normalize
...
;(function(){ if(typeof(from)!=="undefined"&&from){module.exports=from;return from}; return (function(){
  ...
  ;(function(){ var up = Function
    ;((From = function(blueprint){ var that
      ...
      if (!(blueprint && blueprint.sandbox))  that.sandbox  = new(Object)()
      if (!(blueprint && blueprint.incoming)) that.incoming = new(Object)()
  ...
  compileScript = function(source, filename){ var script;
    try { script = vm.createScript(source.replace(/^\#\!.*\n/, '')) }
  ...
  // FIXME: Refactor.
  executeScript = function(script, outgoing, injections){ var
    from = new(From)({ filename:script.filename, incoming:outgoing })
  , sources = [ from.incoming
              , global
              , { incoming: from.incoming
                , global: from.sandbox
                , from: from
                , require: legacyRequire(from)
                , __filename: from.filename }
              , injections ]
    for (source in sources)
      for (variable in sources[source]) if (sources[source].hasOwnProperty(variable))
        from.sandbox[variable] = sources[source][variable]
    return script.runInNewContext(from.sandbox) }
  ...
  Acquisition.prototype.dispatch = function(){ var that = this, iterate, jterate
    if (that.dispatched) return true
        that.dispatched = 0
    
    ;(iterate   = function(index){ var group = that.groups[index]
      if   (index >= that.groups.length) return
      ;(jterate = function(jndex){ var query = group.queries[jndex]
        if (jndex >= group.queries.length) return iterate(index + 1)
        execute.call(that, query, that.from.rootMutators, group.exports, null
                    , function(e, result){ if (e) throw e
          if (group.imports) {
               that.from.imports[query]       = new(Object)()
            Object.keys(group.imports).forEach(function(here){ var there = group.imports[here]
               that.from.imports[query][here] = result[there]
               that.from.sandbox       [here] = result[there] }) }
          else{that.from.imports[query]       = result
               that.from.sandbox[query]       = result }
          
          if (--that.dispatched === 0)
            that.complete() })
        ++that.dispatched
        return jterate(jndex + 1) })(0) })(0)
    
    return true }
  
  Acquisition.prototype.complete = function(){ var that = this
    args = that.groups.reduce(function(args, group){
      args.push.apply(args, group.queries.reduce(function(args, q){
        args.push.apply(args, group.imports? Object.keys(group.imports).map(function(key){
          return that.from.imports[q][key] }) : [that.from.imports[q]])
        return args }, new(Array)())); return args }, new(Array)())
    
    that.callbacks.forEach(function(callback){ return callback.apply(undefined, args) })
  ...
  ;(function(){ var up = Function
    ;((ResolutionMutator = function(body, blueprint){ var that
      ...
      if (typeof(body) === "function") that.body = function(args){ return body.apply(this, args) }
  ...
  , $$ = function($){ var $ = $.slice(0)
      ...
      if (!untestedPaths.empty)
        return probe(untestedPaths.first.path, function(error, extant){ if (!error) return _(u, extant, $); var s
          console.error( (s=new Error().stack.split('\n').length + 10 +
                            $.reduce(function(acc,_,i,a){return acc+(i==a.length-1?'├─':'│ ')+'\033['+(36-i)+'m'},'')+
                            '┐'+$[-1].mutator.tag+": ")+new Array(30-s.replace(/\033\[\d+m/g,'').length).join(' ')+
                          '▌\033[0m'+untestedPaths.first.path.replace(query, "\033[35m"+query+"\033[0m"))
  ...
  /* Prepends the path with each repository from `from.repositories`. */        ;From.prototype.muts.repository =
  repositoryMut = new(ResolutionMutator)(function(path, query, _){
    return _(u, (this.repositories || []).map(function(repository){ return repository+"/"+path })) }
  ...
  relativeMut = new(ResolutionMutator)(function(path, query, _){ var e
    ...
    return _(u, [pfad.dirname(this.filename)+"/"+path]) }
  , { after:[homeMut, extensionMut, initializerMut, libMut], tag:"relative" })
  ...
  execute = function(pathBit, mutators, outgoing, injections, _){
    compile.call(this, pathBit, mutators, function(e, script){ if (e) return _(e)
      process.nextTick(function(){ _(u, executeScript(script, outgoing, injections)) }) }) }
  ...
  from = function(args){
    ...
    ,    that = is_a? this : new(Acquisition)({ from:args.callee })
    
    that.groups.push({ queries: Array.prototype.slice.call(args) })
  ...
  acquisition = function(args){ var _ = args[0], that = args.callee
 ...
 _export = function(exports){
    this.groups.last.exports = exports
    return this }                                                                 ;Acquisition.prototype.import =
 _import = function(imports){
    this.groups.last.imports = imports
    return this }
  ...
  /* This, if defined as an array of paths, will represent repositories that should be searched for
     modules (think ‘global node_modules folders.’) */                             ;From.prototype.repositories =
