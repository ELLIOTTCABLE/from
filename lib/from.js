//_([Skip ahead for the API documentation.](                                                                          #section-3))_
// 
...
// 
/*  ¹ Most ‘`require()`-compatible modules’ are fairly agnostic, as long as you assume they
 ...
 *    with `from.js`.
 *  
 *  ² Some might disagree (;. `from.js` is written from the perspective that abstraction
 *    authors should be able to “return” any value(s) they wish from their code to the
 *    consumer's code, and not be restricted to an `exports` object dictated by the system.
 *  
 *  ³ When synchronicity is at all possible, that is. Obviously, some things *require*
 *    asynchronicity. Such things would be entirely impossible under `require()`, and are
 *    allowed within `from.js`; they therefore force `from.js`'s asynchronous mode upon you.
 */
...
/* My code is written in a style many of you will consider unfamiliar. Don't despair! Though the                  //|   // Code and other content beyond column 120 is not intended for the casual reader, and should be
   ...
   try, I think you'll find you like it. (; */
                                                                                                                  /*|   /* (Things stored in the `OTHER` namespace are intended to be accessed by `test/from.test.js`,
/* (One caveat: This code has to work both client-side and server-side, and all the way back to IE6               /*|   /*  to which this file is concatenated at test-time.) */                                           /*
    and friends. Because of this, a lot of wonderful hacks aren't possible; the code within this                  /*|*/ var From, OTHER = new Object                                                                                 /*
    file is more obtuse than would otherwise be the case, for these reasons.) */                                  /*|*/;(function(){
                                                                                                                  /*|*/ var undefined, u
   // High-level API
   // --------------
   // This library's API is divided into three sections: there's a high-level API, consisting of
   // the `from()` function available to ... // FIXME: finish
                                                                                                                  /*|*/ var $$$ = function(){
   var from
                                                                                                                  /*|*/ var $$$ = function(){
   // ### `from()`
   ...
   // Low-level API
   // -------------
   var /* shadowed: From, */ Acquisition
   
   // The `from()` functions exposed within acquired files, or upon `require()`ing this file, are
   // actually instaces of a `Function` subclass. These `from()` instances are generated by this
   // `From` constructor.
   // 
   // This constructor takes ... // FIXME: document
   From = function( sandbox,  incoming,  imports){ var arguments
    /*  = argument('sandbox','incoming','imports',     arguments */
      
      this.sandbox  = arguments.sandbox  || new Object
      this.incoming = arguments.incoming || new Object
      this.imports  = arguments.imports  || new Object }
                                                                                                                  /*|*/ From = subclass(Function, From, function(){ return from.call(this, arguments) })
                                                                                                                  /*|*/ From.plumbing =
   // Plumbing
   // --------
   ...
   plumbing = {}                                                                                                  /*|*/;plumbing.runInNewContext = runInNewContext; plumbing.isClient = isClient
                                                                                                                  /*|*/ $$$() }
   /* All of these declarations will be hoisted to the beginning of their closure, resulting in
   /* their visibility inside the sub-enclosures responsible for defining From, above. */
   var /* Utility: */ isClient, runInNewContext, hasPrototypeAccessors, subclass
   ...
   runInNewContext.client = function(source, sandbox){ var inner = new Object  
      ...
      body(html(window.document))
         .insertBefore(inner.frame)
                       ...
                       inner.script.text = "window.__fromResult  = "
                                     +"with(window.__fromSandbox) eval(window.__fromSource)"
                       inner.               window.__fromSource  = source
                       inner.               window.__fromSandbox = sandbox
      body(html(inner.document))
         .insertBefore(inner.script)
      
      return inner.window.__fromResult }
   
   runInNewContext.server = function(source, sandbox){
      return require('vm').createScript(source).runInNewContext(sandbox) }
   ...
   hasPrototypeAccessors = function(){ var a, b
             ;(a={}).inherits = true
             ;(b={}).__proto__ = a
      return !!b    .inherits }()
    ...
    * choose which to employ based on the capabilities of the hosting JavaScript implementation. */               /*|*/ OTHER.subclass = 
   subclass = function(up, constructorBody, runtimeBody, intactArguments){
      if (up !== Function) // FIXME: NYI
         throw new RangeError("subclass() may only be called with `Function`")
      if (hasPrototypeAccessors) return subclass.withAccessors .apply(this, arguments)
                            else return subclass.withAliens    .apply(this, arguments) }
   ...
   subclass.withAccessors = function(up, constructorBody, runtimeBody, intactArguments){ var
      constructor = function(){ var
         that = function(){
            return (arguments.callee._runtimeBody || runtimeBody || function(){})
   ...
   subclass.withAliens = function(up, constructorBody, runtimeBody, intactArguments){
      up = runInNewContext(up.name)
   ...
   nodeFor = function(type){
      return function(node){
         return node.getElementsByTagName(type)[0]
             || node.insertBefore((node.ownerDocument || node).createElement(type)) } }                           /*|   /* We delay executing $$$(), an enclosure surrounding each successive layer of the API
                                                                                                                  /*|    * until the *end* of the next section; that allows our utility definitions and shims to take      */
   html = nodeFor('html')                                                                                         /*|    * effect *before* the majority of the library-code, while being placed *below* the more           */
   head = nodeFor('head')                                                                                         /*|    * relevant (and readable) code that we present to readers at the top of the file. */
   body = nodeFor('body')                                                                                         /*|*/;$$$()
   
 ;(root = new From)
  .root = true                                                                                                    /*|*/;if (typeof(module) !== "undefined") module.exports = root
   return root                                                                                                    /*|*/ })()
